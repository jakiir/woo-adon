...
function addWhirlCss() {
    ...
    t.id = "romit-spinner-style", t.type = "text/css", t.styleSheet ? t.styleSheet.cssText = whirlCss : t.appendChild(document.createTextNode(whirlCss)), e.appendChild(t)
}

function stopWhirl() {
    overlay && (overlay.className = ""), removeElementById("romit-spinner-style")
}

function buildAppleOverlay() {
    var e = document.createElement("div");
    e.style.height = "100%", e.style.width = "100%", e.style.position = "fixed", e.style.top = "0", e.style.left = "0", e.style.zIndex = 167270, e.id = "romit-overlay", e.style.background = "rgba(0,0,0,0.65)", e.style.backgroundImage = "radial-gradient(rgba(255,255,255,0.25), rgba(255,255,255,0) 570px)", e.className = e.className ? e.className + " romit-whirl" : "romit-whirl", overlay = e
}
...
function getRomitButtons() {
    for (var e = document.getElementsByTagName("button"), t = [], r = 0; r < e.length; r++) "romit" == e[r].dataset.payment && t.push(e[r]);
    return t
}
var ROMIT_BUTTON_CLASS = "romit-payment-button",
    clickLock = !1,
    whirlCss = '.romit-whirl{position:relative}.romit-whirl:before{content:"";z-index:1;position:absolute;top:0;right:0;bottom:0;left:0;background-color:#999;opacity:.6}.romit-whirl:after{z-index:2;content:"";height:40px;width:40px;position:absolute;top:50%;left:50%;margin:-20px 0 0 -20px;-webkit-transition:all .75s ease 0s;transition:all .75s ease 0s;border-radius:100%;border-top:4px solid #555;-webkit-animation:standard .75s infinite linear;animation:standard .75s infinite linear;-webkit-transform-origin:center;-ms-transform-origin:center;transform-origin:center}.romit-whirl.no-overlay:before{content:none;display:none}@-webkit-keyframes standard{from{-webkit-transform:rotate(0deg);transform:rotate(0deg)}to{-webkit-transform:rotate(360deg);transform:rotate(360deg)}}@keyframes standard{from{-webkit-transform:rotate(0deg);transform:rotate(0deg)}to{-webkit-transform:rotate(360deg);transform:rotate(360deg)}}',
    a = document.createElement("a");
a.href = document.getElementById("romit-payments-script").src;
var
	//thisHost = a.protocol + "//" + a.host,
	thisHost = "https://checkout.sandbox.romit.io",
    thisPort = a.port,
...
iframe.src = thisHost + "/payment-button", iframe.style.height = "100%", iframe.style.width = "100%", needsAppleHack() ? iframe.style.position = "absolute" : iframe.style.position = "fixed", iframe.style.top = "0", iframe.style.left = "0", iframe.style.zIndex = 2147483647, iframe.id = "romit-frame", iframe.sandbox = "allow-same-origin allow-scripts allow-popups", iframe.setAttribute("allowtransparency", "true"), iframe.onload = function() {
    stopWhirl(), clickLock = !1, iframeElementWindow = document.getElementById("romit-frame").contentWindow;
    var e = "*";
    ...
    if (e.source == iframeElementWindow) switch (e.data) {
        case "closeMe":
            removeElementById("romit-overlay"), removeElementById("romit-frame");
            break;
        case "onSuccess":
            Romit.onSuccess(_params.paymentId || "?");
            break;
        case "onFailure":
            Romit.onFailure();
            break;
...
var Romit = {	isTest:0,
    initialize: function(e, t, r, a, i, g) {
		...
		thisHost = (Romit.isTest===0?"https://checkout.sandbox.romit.io":"https://checkout.romit.io");

        for (var o = getRomitButtons(), n = 0; n < o.length; n++) o[n].classList.contains(ROMIT_BUTTON_CLASS) || o[n].classList.add(ROMIT_BUTTON_CLASS), o[n].dataset.style && "default" != o[n].dataset.style || ( /*o[n].style.backgroundImage="url("+thisHost+"/app/img/romit_button_2x.png)",*/ o[n].style.backgroundSize = "cover", o[n].style.width = "236px", o[n].style.height = "51px", o[n].style.backgroundColor = "transparent", o[n].style.border = "none"), o[n].addEventListener("click", function(e) {
